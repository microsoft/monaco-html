  ...
  provideHover(
    ...
    return this._worker(resource)
      ...
      .then(info => {
        ...
        let test = toMarkedStringArray(info.contents);
        ...
        let scriptInfo = info as any;

        if (scriptInfo.documentation) {
          const documentation = displayPartsToString(scriptInfo.documentation);
...
export class DocumentLinkAdapter implements monaco.languages.LinkProvider {
  ...
  public provideLinks(
    model: monaco.editor.IReadOnlyModel,
    token: CancellationToken
  ): Thenable<monaco.languages.ILinksList> {
    const resource = model.uri;
...
export class FoldingRangeAdapter
  ...
  public provideFoldingRanges(
    ...
    token: CancellationToken
  ): Thenable<monaco.languages.FoldingRange[]> {
    const resource = model.uri;
